...
| 5 | **Édition blocs + génération par bloc** | **Deux modes** : **Architecture** (ajout, position, dimensions des blocs) et **Édition** (clic bloc → popup prompt avec **détection des assets**). Images **générées dans les blocs**. Voir `Edition_Panel_Blocs_Bulles.md`, `Edition_Panel_Deux_Modes.md`. | 📋 À faire |
...
- **Deux modes (Architecture / Édition)** : `Edition_Panel_Deux_Modes.md` — mode Architecture (position, dimensions des blocs), mode Édition (prompt avec détection assets, bulles, effets, fond, texte)

...
/** Rendu SVG : un seul path continu (corps + tige) pour cohérence au redimensionnement. */
function SpeechBubbleShape(props: {
  ...
  // Parole : ovale + queue en un seul contour (tige soudée à l’ovale)
  // Ovale coupé en bas ; la queue remplace un segment, contour continu.
  if (type === "speech") {
    const d = "M 42 95 A 48 46 0 1 1 58 95 L 22 118 L 42 95 Z";
    return <path d={d} fill={fill} stroke={stroke} strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round" />;
  }

  if (type === "whisper") {
    const d = "M 42 95 A 48 46 0 1 1 58 95 L 22 118 L 42 95 Z";
    return <path d={d} fill={fill} stroke={stroke} strokeWidth={sw} strokeDasharray="5 4" strokeLinejoin="round" strokeLinecap="round" />;
  }

  // Pensée : nuage (un path : contour nuage + chaîne de bulles en un seul tracé continu)
  if (type === "thought") {
    const d =
      "M 50 20 C 78 20 92 38 88 56 C 94 62 94 74 86 78 C 92 88 82 96 68 92 C 72 98 58 100 48 94 C 38 100 24 94 28 86 C 14 82 12 70 22 62 C 16 44 32 24 50 20 Z " +
      "M 48 94 C 42 98 38 104 36 110 C 34 116 34 118 34 118 C 38 114 44 108 48 94 Z";
    return <path d={d} fill={fill} stroke={stroke} strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round" />;
  }

  // Narration : rectangle arrondi, pas de queue (un seul rect = un contour)
  if (type === "narration") {
    return (
  ...
  // Cri : explosion (zigzag) + queue en éclair en un seul path (tige soudée au corps)
  if (type === "shout") {
    const n = 14;
    const pts: string[] = [];
    for (let i = 0; i < n; i++) {
    ...
    const tailInsert = " L 20 108 L 28 112 L 16 118 L 32 120 L 32 112 L 26 106 L 34 96";
    const body = `M ${pts.slice(0, 7).join(" L ")}${tailInsert} L ${pts.slice(7).join(" L ")} Z`;
    const starPoints: string[] = [];
    for (let i = 0; i < 10; i++) {
      const a = (i / 10) * 2 * Math.PI - Math.PI / 2;
      const r = i % 2 === 0 ? 4 : 1.8;
      starPoints.push(`${84 + r * Math.cos(a)},${12 + r * Math.sin(a)}`);
    }
    const star = `M ${starPoints.join(" L ")} Z`;
    return (
      <>
        <path d={body} fill={fill} stroke={stroke} strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round" />

...
- **Automatisant la génération visuelle** via l'IA (FLUX.1 Schnell pour les utilisateurs Free, FLUX.2 Pro / Pro Edit pour les utilisateurs Pro, via FAL.ai)
- **Garantissant la cohérence stylistique** grâce à un système de templates de style et d'images de référence (Pro)
- **Réduisant drastiquement le temps de production** (de semaines à heures)
- **Rendant la création accessible** à tous, sans compétences artistiques, avec un plan gratuit généreux
...
- **Images de référence** (Plan Pro uniquement) : Upload de 2 images maximum pour guider l'IA
  - Texte explicatif : "2 images de référence pour la cohérence graphique de votre projet"
  - Format portrait/webtoon recommandé
  - Gestion multi-images (ajout/suppression)
  - Stockage dans Supabase Storage
  - Utilisées par FLUX.2 Pro Edit (multi-référence)
...
**Vues multiples** (Plan Pro uniquement) :
- **Vue de face** : Image principale (générée automatiquement, tous les plans)
- **Profil gauche** : Vue de profil gauche (Pro)
- **Profil droite** : Vue de profil droite (Pro)
- **Vue de dos** : Vue arrière du personnage (Pro)
...
- Les utilisateurs Free voient un message d'upgrade pour les vues supplémentaires

**Valeur** : Réutilisation des personnages sous différents angles, cohérence visuelle maximale.

#### 3.4 Opérations sur les assets

- **Modification** : Dialog d'édition pré-rempli (nom + prompt). Si le prompt change, choix entre sauvegarder sans régénérer ou sauvegarder et régénérer l'image.
- **Régénération** : Régénérer une image avec le style actuel du projet (confirmation requise)
- **Suppression** : Supprimer un asset de la bibliothèque (confirmation requise, irréversible)
- **Affichage** : Grille responsive avec aperçu des images
- **Placeholder** : Affichage d'un placeholder pendant la génération
- **Actions au survol** : Boutons Modifier (violet), Régénérer (ambre) et Supprimer (rouge) apparaissent au hover de chaque carte
...
**Description** : Génération automatique d'images via l'IA avec sélection de modèle selon le plan utilisateur.

#### 5.1 Architecture technique

- **Edge Function Supabase** : `generate-asset-image`
- **Modèles IA** :
  - **Free** → FLUX.1 Schnell via `fal-ai/flux/schnell` (~$0.003/image)
  - **Pro** → FLUX.2 Pro via `fal-ai/flux-2-pro` (~$0.03/image)
  - **Pro + refs** → FLUX.2 Pro Edit via `fal-ai/flux-2-pro/edit` (~$0.09/image avec 2 refs)
- **Format** : Images 1024×1024px en PNG
- **Stockage** : Supabase Storage (bucket `dreamweave`)
- **Robustesse** : `fetchWithTimeout(120s)`, `fetchWithRetry(2x, backoff exponentiel)`

#### 5.2 Processus de génération

1. **Vérification des quotas** : Plan utilisateur + comptage usage mensuel
2. **Prompt utilisateur** : Description de l'asset/panel
3. **Enrichissement automatique** :
   ...
   - Ajout des instructions pour les images de référence (si Pro + refs)
   - Ajout des instructions pour les vues spécifiques (profil, dos, etc.)
4. **Sélection du modèle** : Selon le plan (Schnell/Pro/Pro Edit)
5. **Appel API FAL.ai** : Génération de l'image
6. **Upload Storage** : Supabase Storage avec cache-buster (`?v=timestamp`)
7. **Mise à jour BDD** : URL de l'image + enregistrement dans table `usage`
...
**Description** : Vue d'ensemble de l'activité utilisateur avec suivi de consommation.
...
- Statistiques temps réel :
  - Nombre de projets
  - Nombre d'assets
  - Générations du mois (compteur / limite)
- Barre de progression visuelle (usage mensuel)
- Badge du plan actif (Free / Pro) avec lien vers la page Plans
- Liste des projets récents (6 derniers)
- Accès rapide à la création de projet
- Message d'upgrade contextuel pour les utilisateurs Free

**Valeur** : Vue d'ensemble, navigation rapide, transparence sur la consommation.
...
**Fonctionnalités** :
- Affichage du nom d'affichage et de l'email
- Modification du nom d'affichage
- Date de création du compte
- Lien vers la page Plans

**Valeur** : Personnalisation, gestion de compte.

---

### 11. 💳 Plans & Pricing

**Description** : Système de monétisation avec deux plans (Free / Pro).
...
| **Vues multiples** | Face seule | 4 vues (face, profil G/D, dos) |
| **Résolution** | 1024×1024 | 1024×1024 |

#### Mécanisme de contrôle

Le quota mensuel de générations est le seul mécanisme de limitation :
- Chaque appel à l'API FAL.ai (= 1 génération) incrémente un compteur dans la table `usage`
- Le compteur est remis à zéro au début de chaque mois calendaire
- Les projets et assets ne sont **pas limités** car leur création ne consomme pas de ressources IA
- Le backend (Edge Function) vérifie le quota avant chaque génération et rejette avec HTTP 429 si dépassé

#### Fonctionnalités de la page Plans

- Comparaison côte-à-côte des plans
- Tableau des fonctionnalités détaillé
- Indicateur du plan actuel + usage
- Bouton de changement de plan
- Intégration Stripe prévue (actuellement changement direct en BDD)

**Valeur** : Transparence, conversion Free → Pro, monétisation.

---

### 12. 🌐 Page d'accueil (Landing)
...
1. **Cohérence stylistique garantie** : Système de templates de style + images de référence
2. **Vues multiples des personnages** : Génération de profil, dos, etc. (Pro)
...
8. **Multi-modèles IA** : Sélection automatique du modèle selon le plan
...
- **State Management** : TanStack React Query 5 (cache serveur, mutations, invalidation)
- **Architecture** : Service layer + hooks React Query + types centralisés
...
Le produit est en phase MVP achevée (Phase 1 complète) :
- ✅ Gestion de projets, assets, style — opérationnels
- ✅ Système de plans Free / Pro avec quotas mensuels — opérationnel
- ✅ Multi-modèles IA (Schnell / FLUX.2 Pro / Pro Edit) — opérationnel
- ✅ Profil utilisateur et page Plans — opérationnels
- 📋 Fonctionnalités avancées (panels, dialogues, chapitres) — Phase 2 planifiée
- 📋 Intégration Stripe — planifiée

...
**État d'avancement** : **Étape 1** = ✅ livrée (février 2026). **Étapes 2 à 7** = 📋 à faire. Voir § 3 (synthèse) et § 4 (tests à faire).
...
### Étape 2 — Lien chapitre textuel ↔ chapitre visuel (double visualisation) 📋 À faire
...
### Étape 3 — Découpage du chapitre textuel en panels (construire le chapitre visuel) 📋 À faire
...
### Étape 4 — Liaison découpage ↔ Édition de l'œuvre (section Scénario) 📋 À faire
...
### Étape 5 — Génération panel par panel (mode Automatique) 📋 À faire
...
| 2 | Lien textuel ↔ visuel (double visualisation) | Chapitre texte à gauche (Aperçu = surbrillance assets + hover), panels à droite ; lien `linked_scenario_chapter_id`. Pas de panneau Assets dédié. | 📋 À faire |
...
| 4 | Liaison découpage ↔ Édition de l'œuvre | Découpage depuis la section Scénario ; import vers chapitre visuel (création des panels). | 📋 À faire |
| 5 | Génération (mode Auto) | **Blocs agencés automatiquement** selon le scénario ; images générées par bloc ; prompt = style + assets + **contexte chapitre** + description. Panel 720×5000. | 📋 À faire |
...
| **2** | Affichage chapitre texte à gauche, panels à droite ; Aperçu = surbrillance assets + hover (même rendu que Scénario) ; lien `linked_scenario_chapter_id` (sélecteur si non lié) ; création chapitre avec suggestion par numéro, message si aucun chapitre textuel. |
...
| **5** | Génération par bloc (bouton par bloc) ; prompt = style + assets + contexte chapitre + description ; dimensions 720×5000 ; pas de génération « tout le chapitre » ; régénération / édition. |
...
**Note** : À la ligne « Dimensions (obligatoires) » (Étape 5), corriger manuellement la typo : remplacer « l'image d'un En mode Auto » par « En mode Auto » (phrase correcte : « En mode Auto les blocs sont agencés automatiquement »).

...
- **Visualisation de la case** : en édition, chaque case est affichée en visualisation immersive (plein écran), avec canvas central 800×H. **Les images sont générées dans les blocs.** L'édition est organisée par sous-menus visuels (Personnalisation, Couleurs, Dialogue). Voir `Edition_Case_Blocs_Bulles.md` et **`Edition_Case_Deux_Modes.md`**.

Références : `07_Roadmap_Produit.md` Phase 2, `11_Rapport_Chapitres_Flux_Blocs_Scenario.md`, `UX.md` § 3.2, **`Edition_Case_Blocs_Bulles.md`** (édition blocs + bulles, workflow, visu 800×H), **`Edition_Case_Deux_Modes.md`** (sous-menus d'édition).
...
| **Bulles, effets, fond, texte** | Voir Étape 7 et `Edition_Case_Deux_Modes.md`. |
...
| 5 | **Édition blocs + génération par bloc** | **Vue unifiée immersive** : canvas 800×H + sous-menus latéraux. Ajout, position, dimensions des blocs (glisser-déposer, poignées) ; prompt par bloc ; génération par bloc (dimensions = bloc). Voir `Edition_Case_Blocs_Bulles.md`, `Edition_Case_Deux_Modes.md`. | ✅ Livrée |
...
- **Édition case (blocs + bulles)** : `Edition_Case_Blocs_Bulles.md` — workflow, visualisation 800×H, édition blocs, prompts par bloc, génération par bloc, bulles
- **Sous-menus d'édition** : `Edition_Case_Deux_Modes.md` — Personnalisation, Couleurs, Dialogue (vue immersive unifiée).
- **Flux et règles** : `11_Rapport_Chapitres_Flux_Blocs_Scenario.md` — § 2 (flux Auto/Structuré), § 3.4 (double visualisation), § 3.5 (IA Case)
